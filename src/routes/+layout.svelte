<script lang="ts">
    ...
    import { Library, Play, Pause, Volume2, GripVertical, Search } from 'lucide-svelte';
...
<div class="h-screen flex flex-col bg-background">
  <div class="flex flex-1 overflow-hidden">
    <!-- Resizable Sidebar -->
    <div class="bg-card flex flex-col gap-2 p-2 relative" style="width: {sidebarWidth}px;">
        ...
        >
            <GripVertical size={16} class="text-muted-foreground group-hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
        </div>
    </div>
    ...
    <div class="flex items-center justify-between max-w-screen-xl mx-auto">
      ...
      <div class="flex items-center gap-4 w-1/3">
        ...
        <div>
          ...
          <h4 class="text-sm font-medium">{trackParam || 'No track playing'}</h4>
          <p class="text-xs text-muted-foreground">
            {trackParam ? 'Playing your selected track' : 'Select a track to play'}
          </p>
        </div>
      </div>
